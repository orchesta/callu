...
public class MaintenanceWindowsController(IMaintenanceWindowService maintenanceService) : ControllerBase
{
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateMaintenanceWindowRequest request, CancellationToken ct)
    {
        var item = await maintenanceService.UpdateAsync(id, request, ct);
