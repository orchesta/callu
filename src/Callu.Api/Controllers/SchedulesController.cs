    ...
    INotificationPushService pushService) : ControllerBase
{
    ...
    /// <summary>Applies a schedule's settings and its whole rotation list as one unit — one transaction, one
    /// rematerialization, so no intermediate rota is ever published. Prefer it over chaining the granular endpoints.</summary>
    [HttpPut("{id:guid}/plan")]
    [Authorize(Policy = Policies.CanManageSchedules)]
    public async Task<IActionResult> SavePlan(Guid id, [FromBody] SaveSchedulePlanRequest request, CancellationToken ct)
    {
        var success = await scheduleService.SaveSchedulePlanAsync(id, request, ct);
        if (!success) return NotFound();
        await pushService.BroadcastScheduleUpdatedAsync(id, ct);
        return NoContent();
    }
    ...
    {
        var status = await onCallService.GetCurrentOnCallAsync(id, cancellationToken: ct);
        return Ok(status);
    }
    ...
    [Authorize(Policy = Policies.CanViewSchedules)]
    public async Task<IActionResult> GetCoverage(Guid id, [FromQuery] int days = 30, CancellationToken ct = default)
    {
        // Unbounded days would scan a large occurrence window on a small VPS.
        days = Math.Clamp(days, 1, 90);
        var coverage = await rotationService.ValidateRotationCoverageAsync(id, days, ct);
        return Ok(coverage);
    }
}
