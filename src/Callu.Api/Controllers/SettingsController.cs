...
public class SettingsController(
    ...
    IFirebaseSettingsService firebaseSettingsService,
    ...
    INotificationPushService pushService) : ControllerBase
{
    ...
    [HttpGet("firebase")]
    public async Task<IActionResult> GetFirebaseSettings(CancellationToken ct)
    {
        var settings = await firebaseSettingsService.GetSettingsAsync(ct);
        return Ok(settings);
    }

    [HttpPut("firebase")]
    public async Task<IActionResult> SaveFirebaseSettings(
        [FromBody] UpdateFirebaseSettingsRequest request, CancellationToken ct)
    {
        try
        {
            var success = await firebaseSettingsService.SaveSettingsAsync(request, ct);
            if (!success) return BadRequest(ApiResponse.Fail(Messages.Get("settings.firebaseSaveFailed")));
            await pushService.BroadcastSettingsUpdatedAsync("firebase", ct);
            return Ok(new { message = Messages.Get("settings.firebaseSaved") });
        }
    ...
    [HttpPost("firebase/test")]
    public async Task<IActionResult> TestFirebase(CancellationToken ct)
    {
        var result = await firebaseSettingsService.TestAsync(ct);
        return Ok(result);
    }
