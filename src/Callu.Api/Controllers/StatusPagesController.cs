    ...
    IHealthCheckExecutor healthCheckExecutor) : ControllerBase
{
    ...
    /// <summary>Operator-side removal of a subscriber; a subscriber's own exit is the anonymous token route below.</summary>
    ...
    /// <summary>Double opt-in confirmation; the URL token is the proof of email ownership, and the response is
    /// deliberately identical for confirmed, already-confirmed and unknown tokens.</summary>
    ...
    /// <summary>Per-process salt for <c>HashVisitor</c>, so the stored hash cannot be rainbow-tabled back to a known IP set.</summary>
