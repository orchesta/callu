    ...
    INotificationPushService pushService) : ControllerBase
{
    ...
    {
        await teamService.UpdateTeamAsync(id, request, ct);
    ...
    {
        await teamService.AddMemberAsync(id, request.UserId, request.Role, ct);
        await pushService.BroadcastTeamUpdatedAsync(id, ct);
    ...
    {
        await teamService.RemoveMemberAsync(teamId, memberId, ct);
    ...
    {
        await teamService.UpdateMemberRoleAsync(teamId, memberId, request.Role, ct);
        await pushService.BroadcastTeamUpdatedAsync(teamId, ct);
        return NoContent();
    }
}
