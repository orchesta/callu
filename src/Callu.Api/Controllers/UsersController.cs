    ...
    IProfileService profileService) : ControllerBase
{
    ...
    {
        var (success, error, emailSent, inviteLink) = await userService.InviteUserAsync(request.Email, request.Role, ct);
        if (!success) return BadRequest(ApiResponse.Fail(error ?? "Operation failed"));

        // The account exists either way. Without a working SMTP the link is the only way to reach
        // the invitee, and reporting "sent" would strand them behind an address now taken.
        return Ok(emailSent
            ? new { message = Messages.Get("users.invitationSent"), emailSent, inviteLink }
            : new { message = Messages.Get("users.invitationNotEmailed"), emailSent, inviteLink });
    }
    ...
    {
        var (found, emailSent, inviteLink) = await userService.ResendInvitationAsync(id, ct);
        if (!found) return NotFound();

        return Ok(emailSent
            ? new { message = Messages.Get("users.invitationResent"), emailSent, inviteLink }
            : new { message = Messages.Get("users.invitationNotEmailed"), emailSent, inviteLink });
    }
