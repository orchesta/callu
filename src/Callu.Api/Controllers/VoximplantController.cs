    ...
    ICommunicationProviderRegistry providerRegistry) : ControllerBase
{
    ...
    [HttpGet("{providerId:guid}/call-history")]
    public async Task<IActionResult> GetCallHistory(
        Guid providerId,
        [FromQuery] DateTime? fromUtc,
        [FromQuery] DateTime? toUtc,
        [FromQuery] int count = 50,
        [FromQuery] string? remoteNumber = null,
        [FromQuery] long? callSessionHistoryId = null,
        CancellationToken ct = default)
    {
        var history = await voxService.GetCallHistoryAsync(providerId, new VoxCallHistoryQuery
        {
            FromUtc = fromUtc,
            ToUtc = toUtc,
            Count = count,
    ...
    [HttpGet("{providerId:guid}/call-history/{callSessionHistoryId:long}/log")]
    public async Task<IActionResult> GetSessionLog(
        Guid providerId, long callSessionHistoryId, CancellationToken ct)
    {
        var log = await voxService.GetSessionLogAsync(providerId, callSessionHistoryId, ct);
        return Ok(log);
    }
}
