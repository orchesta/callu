...
{
    /// <summary>Workspace-wide inventory of webhook API keys; the route deliberately skips the <c>{serviceId}</c> prefix.</summary>
    ...
    /// <summary>Get webhook settings for a service; reads drop to <c>CanViewServices</c> because viewing config needs no manage rights.</summary>
    [HttpGet]
    [Authorize(Policy = Policies.CanViewServices)]
    public async Task<IActionResult> GetSettings(Guid serviceId, CancellationToken ct)
    {
        var canManage = User.HasClaim("CanManageServices", "true");
        var settings = await webhookConfig.GetWebhookSettingsAsync(serviceId, canManage, ct);
    ...
    /// <summary>
    /// Set the HMAC signature secret and header name; the secret is returned exactly once.
    /// </summary>
    [HttpPost("signature")]
    ...
    /// <summary>
    /// Remove the HMAC signature secret, leaving webhooks unsigned.
    /// </summary>
