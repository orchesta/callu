...
/// <summary>Maps domain exceptions to HTTP status codes and the <c>ApiResponse</c> envelope.</summary>
public class GlobalExceptionHandler(ILogger<GlobalExceptionHandler> logger) : IExceptionHandler
{
        ...
        HttpContext httpContext, Exception exception, CancellationToken cancellationToken)
    {
        ...
        // Method and path, never the query string: a 4xx nobody can attribute to a route is a
        // report an operator cannot act on, and query strings carry tokens.
        ...
        {
            logger.LogWarning("Handled exception ({StatusCode}) on {Method} {Path}: {Message}",
                (int)statusCode, httpContext.Request.Method, httpContext.Request.Path, exception.Message);
        }
