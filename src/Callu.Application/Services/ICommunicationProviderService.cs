...
public interface ICommunicationProviderService
{
    ...
    /// <summary>Every routable capability, what it is pinned to, and what it could be pinned to.</summary>
    Task<IEnumerable<CapabilityRouteDto>> GetCapabilityRoutesAsync(CancellationToken cancellationToken = default);

    /// <summary>Pins a capability to one provider; a null provider clears the pin.</summary>
    Task SetCapabilityRouteAsync(
