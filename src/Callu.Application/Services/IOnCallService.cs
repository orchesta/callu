...
public interface IOnCallService
{
    ...
    /// </summary>
    Task<OnCallStatusDto?> GetCurrentOnCallAsync(Guid scheduleId, bool bypassCache = false, CancellationToken cancellationToken = default);

    /// <summary>
    /// Get current on-call user for a team (across all schedules)
    /// </summary>
    Task<OnCallStatusDto?> GetCurrentOnCallForTeamAsync(Guid teamId, bool bypassCache = false, CancellationToken cancellationToken = default);
