...
public interface IVoximplantManagementService
{
    ...
    Task<VoxCallHistoryDto> GetCallHistoryAsync(Guid providerId, VoxCallHistoryQuery query, CancellationToken cancellationToken = default);

    Task<VoxSessionLogDto> GetSessionLogAsync(Guid providerId, long callSessionHistoryId, CancellationToken cancellationToken = default);
}
