...
public class TtsPreviewRequestValidator : AbstractValidator<TtsPreviewRequest>
{
    public TtsPreviewRequestValidator()
    {
        RuleFor(x => x.Segments)
            .NotEmpty().WithMessage("At least one segment is required")
            .Must(s => s.Count <= TtsPreviewLimits.MaxSegments)
            .WithMessage($"At most {TtsPreviewLimits.MaxSegments} segments");

        RuleForEach(x => x.Segments).ChildRules(segment =>
        {
            segment.RuleFor(s => s.Text)
                .NotEmpty().WithMessage("Segment text is required")
                .MaximumLength(TtsPreviewLimits.MaxSegmentChars);

            segment.RuleFor(s => s.Lang)
                .NotEmpty().WithMessage("Segment language is required")
                .MaximumLength(TtsPreviewLimits.MaxLanguageChars);
        });

        RuleFor(x => x.Voice)
            .MaximumLength(TtsPreviewLimits.MaxVoiceChars);
    }
}
