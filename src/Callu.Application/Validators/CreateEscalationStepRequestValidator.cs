...
public class CreateEscalationStepRequestValidator : AbstractValidator<CreateEscalationStepRequest>
{
    public CreateEscalationStepRequestValidator()
    {
        RuleFor(x => x.Title)
            ...
            .MaximumLength(100).WithMessage("Step title cannot exceed 100 characters");

        RuleFor(x => x.DelayMinutes)
            .GreaterThanOrEqualTo(0).WithMessage("Delay must be 0 or greater");

        // 0 (the default, and what every client sends) means "append" — EscalationService derives it.
        RuleFor(x => x.Level)
            .GreaterThanOrEqualTo(0).WithMessage("Level must be 0 or greater");
