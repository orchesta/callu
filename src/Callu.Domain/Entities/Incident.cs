...
public class Incident : BaseEntity
{
    ...
    /// <summary>Column widths, read by the validators and the ingest clip so they cannot drift.</summary>
    public const int MaxTitleLength = 200;
    public const int MaxDescriptionLength = 4000;
    public const int MaxExternalAlertIdLength = 200;
    ...
    /// <summary>Full policy passes completed; used when ExhaustionBehavior is Repeat.</summary>
    public int EscalationCyclesCompleted { get; set; }

    /// <summary>
    /// When set, org-level channel notification dispatch (Slack/Teams/webhook/email) is skipped.
    /// </summary>
    public bool IsNotificationSuppressed { get; set; } = false;

    /// <summary>
    /// When set, per-responder paging (auto-escalation) was suppressed at creation by an alert rule.
    /// </summary>
    public bool IsPagingSuppressed { get; set; } = false;

    /// <summary>
    /// When the escalation trigger was staged (an active policy was found and the trigger published).
    /// </summary>
    public DateTime? EscalationStagedAt { get; set; }

    /// <summary>
    /// When the current escalation step's page first failed to be queued; null while dispatch is healthy.
    /// </summary>
    ...
    /// </summary>
    [StringLength(MaxExternalAlertIdLength)]
    ...
    /// <summary>Moves this incident into Investigating, from Open, Acknowledged or Mitigated.</summary>
    public void StartInvestigation(string userId)
    {
        if (Status != IncidentStatus.Open && Status != IncidentStatus.Acknowledged && Status != IncidentStatus.Mitigated)
    ...
    public void Reopen(string userId)
    {
        ...
        DispatchFailingSince = null;

        // Both flags belong to the episode that just closed, not to the alert rule that set them.
    ...
    /// <summary>True when the incident is in a terminal state; prefer this over comparing the enum numerically.</summary>
