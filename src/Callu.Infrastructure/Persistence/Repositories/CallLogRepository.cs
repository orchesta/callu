    ...
    : Repository<CallLog>(context, logger), ICallLogRepository
{
    ...
    public Task<bool> AnyWithCallTokenAsync(string callToken, CancellationToken cancellationToken = default) =>
        _dbSet.AsNoTracking().AnyAsync(cl => cl.CallToken == callToken, cancellationToken);

    public Task<bool> AnyForAttemptAsync(Guid attemptId, CancellationToken cancellationToken = default) =>
        _dbSet.AsNoTracking().AnyAsync(cl => cl.AttemptId == attemptId, cancellationToken);
}
