    ...
    : Repository<RefreshToken>(context, logger), IRefreshTokenRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        var affected = await _dbSet
            ...
            .ExecuteUpdateAsync(setters => setters
                ...
                .SetProperty(t => t.ReplacedByTokenHash, replacedByTokenHash), cancellationToken);
    ...
    public async Task<int> RevokeActiveByFamilyIdAsync(
        Guid familyId,
        string reason,
        CancellationToken cancellationToken = default)
    {
        ...
        if (revoked > 0)
            logger.LogInformation(
                "Revoked {Count} refresh token(s) in family {FamilyId} (reason: {Reason})",
