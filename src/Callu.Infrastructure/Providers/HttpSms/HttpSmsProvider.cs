...
/// <summary>Generic HTTP SMS provider that sends by rendering an operator-defined request template.</summary>
public class HttpSmsProvider : BaseCommunicationProvider
{
    ...
    public HttpSmsProvider(
        ...
        ILogger<HttpSmsProvider> logger)
    {
    ...
    /// <summary>Binds the config and decrypts the credentials, which are stored encrypted at rest.</summary>
    public override async Task InitializeAsync(string configJson, SipTrunkSettings? sipTrunk)
    {
        ...
        {
            _config.ApiKey = _secretProtector.Unprotect(_config.ApiKey);
            _config.Username = _secretProtector.Unprotect(_config.Username);
            _config.Password = _secretProtector.Unprotect(_config.Password);
        }
    }
    ...
    /// <summary>Replaces every placeholder in <paramref name="template"/>, passing each substituted value
    /// through <paramref name="escape"/> so it is safe for the target location.</summary>
