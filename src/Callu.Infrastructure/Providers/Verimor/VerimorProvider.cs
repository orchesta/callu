...
/// <summary>Verimor SMS provider; credentials go in the JSON body or query params, not Basic Auth.
/// API reference: https://github.com/verimor/SMS-API/blob/master/user_guide.md</summary>
public class VerimorProvider : BaseCommunicationProvider
{
    ...
    public VerimorProvider(
        IHttpClientFactory httpClientFactory,
        ProviderSecretProtector secretProtector,
        ILogger<VerimorProvider> logger)
    {
    ...
    public override async Task InitializeAsync(string configJson, SipTrunkSettings? sipTrunk)
    {
        ...
        {
            _config.ApiUsername = _secretProtector.Unprotect(_config.ApiUsername);
            _config.ApiPassword = _secretProtector.Unprotect(_config.ApiPassword);
        }
    }
    ...
    /// <summary>Sends an SMS via the Verimor POST JSON API.</summary>
    public override async Task<SmsResult> SendSmsAsync(SendSmsRequest request)
    {
        ...
        try
        {
            ...
            using var response = await client.PostAsync($"{VerimorApiBase}/send.json", content);
