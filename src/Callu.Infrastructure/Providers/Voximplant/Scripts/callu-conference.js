/** Callu conference scenario (video + HD audio). CALLU_API_URL / CALLU_API_KEY are injected by the
 *  backend at provisioning time; recording is disabled. */
...
var confData = {};
var callbackUrl = "";
// Applies only when the config fetch below fails, so it has to match the cap the API sends.
...
VoxEngine.addEventListener(AppEvents.CallAlerting, function (e) {
    ...
    if (participants.length >= maxParticipants) {
        // Reject silently with SIP 486 (Busy Here) — no spoken TTS message.
