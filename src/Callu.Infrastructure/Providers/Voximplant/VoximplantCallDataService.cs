...
/// <summary>DB-backed call token service for VoxEngine data fetch; tokens are one-time use and expire
/// after <see cref="TokenExpiry"/>.</summary>
    ...
    VoxSipPasswordProtector sipPasswordProtector) : ICallDataService
{
    ...
    /// <summary>Returns a copy of <paramref name="source"/> with the SIP password encrypted; the caller's
    /// object is left alone so a shared instance cannot leak ciphertext to the wire.</summary>
    ...
    public Task<VoxCallbackResult> ProcessCallbackAsync(
