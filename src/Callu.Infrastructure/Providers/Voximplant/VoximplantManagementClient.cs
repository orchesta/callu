...
/// <summary>HTTP client for the Voximplant Management API, using Bearer auth when a JWT authenticator is
/// supplied and the api_key query string otherwise.</summary>
    ...
    VoximplantJwtAuthenticator? jwtAuthenticator = null)
{
    ...
    /// <summary>Rotates the user's password to a fresh random value for the Web SDK login; nothing revokes
    /// it, so it stays valid until the next join rotates it again.</summary>
    ...
    public async Task<VoxGetCallHistoryResponse> GetCallHistoryAsync(
        DateTime fromUtc,
        DateTime toUtc,
        int count = 50,
        bool withCalls = true,
        ...
        long? callSessionHistoryId = null)
    {
        var args = new Dictionary<string, string>
        {
            ["from_date"] = fromUtc.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss"),
            ["to_date"] = toUtc.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss"),
            ["timezone"] = "Etc/GMT",
            ["count"] = Math.Clamp(count, 1, 1000).ToString(),
            ["desc_order"] = "true",
            ["with_calls"] = withCalls ? "true" : "false",
        };
        if (applicationId.HasValue)
            args["application_id"] = applicationId.Value.ToString();
        if (!string.IsNullOrWhiteSpace(remoteNumber))
            args["remote_number"] = remoteNumber;
        if (callSessionHistoryId.HasValue)
            args["call_session_history_id"] = callSessionHistoryId.Value.ToString();

        return await GetAsync<VoxGetCallHistoryResponse>("GetCallHistory", args);
    }

    public async Task<(bool Ok, string? Body, string? Error)> DownloadUrlAsync(string url, int maxBytes = 512_000)
    {
        try
        {
            ...
            {
                var read = await stream.ReadAsync(buffer.AsMemory(0, Math.Min(buffer.Length, maxBytes - total)));
                if (read == 0) break;
        ...
        {
            logger.LogWarning(ex, "Failed to download Voximplant URL");
