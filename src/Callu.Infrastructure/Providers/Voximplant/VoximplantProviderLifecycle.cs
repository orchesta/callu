...
public class VoximplantProviderLifecycle : ICommunicationProviderLifecycle
{
    ...
    /// <summary>Contract the VoxEngine scripts speak with the API; bump it here and in the scripts'
    /// <c>CALLU_SCRIPT_CONTRACT</c> whenever a script change is needed for callbacks to keep working.</summary>
    internal const string ScriptContractVersion = "1.6";

    private static string ContractMarker => $"CALLU_SCRIPT_CONTRACT = \"{ScriptContractVersion}\"";

    private const string StaleScriptIssue =
        "The provisioned Voximplant scenario is running an older Callu script. It cannot acknowledge " +
        "incidents (its callbacks are refused), so responders who press 1 on the phone are still paged. " +
        "Re-provision to upload the current script.";
    ...
    public async Task<ProvisioningResult> ProvisionAsync(Guid providerId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Reuse the stored key; mint only when there is none. Rotating on every run could leave
            // the cloud and the DB holding different keys, which kills phone acknowledgement silently.
            var storedScenarioApiKey = _secretProtector.Unprotect(config.Provisioning?.ScenarioApiKey);
            var scenarioApiKey = string.IsNullOrWhiteSpace(storedScenarioApiKey)
                ? Guid.NewGuid().ToString("N") + Guid.NewGuid().ToString("N")
                : storedScenarioApiKey;
            ...
            await SaveProvisioningConfig(providerId, config, new ProvisioningConfig
            {
                ...
                ScriptContractVersion = ScriptContractVersion
            }, cancellationToken);
    ...
    public async Task<ProvisioningStatus> GetStatusAsync(Guid providerId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // A scenario id existing says nothing about what the scenario CONTAINS. Reporting
            // "fully provisioned" for a stale script is the dangerous answer: it still rings the
            // responder, but the acknowledgement it sends back is refused, so the operator believes
            // paging works right up until the first unacknowledgeable incident.
            var (scriptIsCurrent, scriptIssue) = await CheckIncidentScriptContractAsync(client, provConfig);
            resources.Add(new ProvisioningResource(
                $"callu-incident-call script (v{ScriptContractVersion} contract)",
                "Scenario script",
                scriptIsCurrent,
                provConfig.IncidentCallScenarioId > 0 ? provConfig.IncidentCallScenarioId : null));
            if (scriptIssue != null) issues.Add(scriptIssue);
    ...
    /// <summary>Whether the scenario Voximplant is actually running speaks the current callback contract;
    /// an unreachable Voximplant leaves the answer unverified rather than reporting "not provisioned".</summary>
    private async Task<(bool IsCurrent, string? Issue)> CheckIncidentScriptContractAsync(
        VoximplantManagementClient client, ProvisioningConfig provConfig)
    {
        if (provConfig.IncidentCallScenarioId <= 0)
            return (false, null);

        if (!string.Equals(provConfig.ScriptContractVersion, ScriptContractVersion, StringComparison.Ordinal))
            return (false, StaleScriptIssue);

        try
        {
            var response = await client.GetScenariosAsync(
                scenarioId: provConfig.IncidentCallScenarioId,
                withScript: true);

            var script = response.Result?
                .FirstOrDefault(s => s.ScenarioId == provConfig.IncidentCallScenarioId)?
                .ScenarioScript;

            if (string.IsNullOrEmpty(script))
                return (true, "Could not read the provisioned scenario script back from Voximplant, so its version was not verified.");

            return script.Contains(ContractMarker, StringComparison.Ordinal)
                ? (true, null)
                : (false, StaleScriptIssue);
        }
        catch (HttpRequestException ex)
        {
            return (true, $"Could not verify the provisioned scenario script: {ex.Message}");
        }
    }
