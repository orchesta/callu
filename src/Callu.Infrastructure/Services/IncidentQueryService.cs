...
/// <summary>Read-only incident queries: dashboard summaries, status counts, external alert lookups.</summary>
    ...
    HybridCache cache) : IIncidentQueryService
{
    ...
    private async Task<Shared.Models.Dashboard.DashboardSummaryDto> BuildDashboardSummaryAsync(int recentCount, int timeRangeDays, CancellationToken cancellationToken)
    {
        ...
        // Closing is how a resolved incident gets archived, so counting it as unresolved made
        // tidying up lower the rate.
        var settledCount = resolvedCount + (statusCounts
            .FirstOrDefault(s => s.Status == IncidentStatus.Closed)?.Count ?? 0);
        ...
        var resolvedRate = totalIncidents > 0 ? (settledCount * 100 / totalIncidents) : 100;
    ...
    {
        // Sub-minute truncates to "0m", which reads as "not measured" and hides the difference
        // between a 5-second acknowledgement and a 55-second one.
        if (minutes < 1) return $"{(int)Math.Round(minutes * 60)}s";
