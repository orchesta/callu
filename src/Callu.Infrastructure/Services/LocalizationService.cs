...
public class LocalizationService : ILocalizationService
{
    ...
    private readonly List<DateTimeZone> _zones;
    ...
    public LocalizationService(IDateTimeZoneProvider tzProvider, IClock clock)
    {
        ...
        _clock = clock;
        _zones = LoadZones(tzProvider);
        _cultures = LoadCultures();
    }

    private static List<DateTimeZone> LoadZones(IDateTimeZoneProvider tzProvider) =>
        tzProvider.Ids
            .Select(tzProvider.GetZoneOrNull)
            .Where(zone => zone is not null)
            .Select(zone => zone!)
            .ToList();

    /// <summary>
    /// Describes the cached zone list, computing UTC offsets per call rather than caching them.
    /// </summary>
    private List<TimezoneDto> DescribeZones()
    {
        var now = _clock.GetCurrentInstant();

        return _zones
            .Select(zone => Describe(zone, now))
            .OrderBy(t => t.BaseUtcOffset)
            .ThenBy(t => t.Id)
            .ToList();
    }

    private static TimezoneDto Describe(DateTimeZone zone, Instant now)
    {
        ...
        {
            Id = zone.Id,
            DisplayName = $"(UTC{FormatOffset(offset)}) {zone.Id}",
            StandardName = zone.Id,
    ...
    public TimezoneDto? GetTimezone(string timezoneId)
    {
        var zone = _zones.FirstOrDefault(z =>
            z.Id.Equals(timezoneId, StringComparison.OrdinalIgnoreCase));

        return zone is null ? null : Describe(zone, _clock.GetCurrentInstant());
    }
