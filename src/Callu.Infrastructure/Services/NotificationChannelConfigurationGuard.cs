...
internal static class NotificationChannelConfigurationGuard
{
    ...
    internal static void EnsureValid(
        ...
        bool notifyOnClosed = false,
        bool notifyOnReopened = false,
        bool allowPrivate = false)
    {
        if (!notifyOnCreated && !notifyOnAcknowledged && !notifyOnResolved && !notifyOnClosed && !notifyOnReopened)
        {
            throw new ValidationException(new Dictionary<string, string[]>
            {
                ["Lifecycle"] = ["Select at least one lifecycle trigger (created, acknowledged, resolved, closed, or reopened)."],
            });
        }
        ...
        switch (type)
        {
            ...
            case NotificationChannelType.MicrosoftTeams:
                RequireHttpUrl(configuration, "webhookUrl", failures, allowPrivate);
                break;
            ...
            case NotificationChannelType.Webhook:
                RequireHttpUrl(configuration, "url", failures, allowPrivate);
        ...
        bool allowPrivate)
    {
        ...
        if (!UrlSanitizer.IsValidHealthCheckUrl(v, allowPrivate))
