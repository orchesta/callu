    ...
    ILogger<PostmortemService> logger) : IPostmortemService
{
        ...
        return await transactionManager.ExecuteInTransactionAsync(async () =>
        {
            ...
            else
            {
                throw new Callu.Shared.Exceptions.BusinessRuleException(
                    $"Postmortem is locked and cannot be modified. Status: '{entity.Status}'.");
            }
    ...
    public async Task<bool> SubmitForReviewAsync(Guid id, CancellationToken ct = default)
        => await ExecuteStateTransitionAsync(id, AuditAction.Submitted, e => e.Submit(), ct);

    public async Task<bool> RejectReviewAsync(Guid id, CancellationToken ct = default)
        => await ExecuteStateTransitionAsync(id, AuditAction.Rejected, e => e.Reject(), ct);

    public async Task<bool> PublishAsync(Guid id, CancellationToken ct = default)
        => await ExecuteStateTransitionAsync(id, AuditAction.Published, e => e.Publish(), ct);

    public async Task<bool> LockAsync(Guid id, CancellationToken ct = default)
        => await ExecuteStateTransitionAsync(id, AuditAction.Locked, e => e.Lock(), ct);

    private async Task<bool> ExecuteStateTransitionAsync(Guid id, AuditAction action, Action<Postmortem> transition, CancellationToken ct)
    {
        return await transactionManager.ExecuteInTransactionAsync(async () =>
        {
            var entity = await repo.GetByIdAsync(id, ct);
    ...
    public async Task<bool> DeleteAsync(Guid id, CancellationToken ct = default)
    {
        return await transactionManager.ExecuteInTransactionAsync(async () =>
        {
            ...
            if (entity.Status != PostmortemStatus.Draft)
                throw new Callu.Shared.Exceptions.BusinessRuleException(
                    $"Only Draft postmortems can be deleted. Current: '{entity.Status}'.");
