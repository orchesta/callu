    ...
    HybridCache cache) : IReportingService
{
        ...
        DateTime from, DateTime to, string groupBy, CancellationToken ct)
    {
        ...
        {
            "week" => incidents.GroupBy(i => WeekStart(i.CreatedAt)),
        ...
        DateTime from, DateTime to, CancellationToken ct)
    {
        ...
        var byWeek = incidents.GroupBy(i => WeekStart(i.CreatedAt));
    ...
    /// <summary>The first day of the 7-day bucket a timestamp falls in, counted from January 1st.</summary>
    private static DateTime WeekStart(DateTime timestamp)
    {
        var daysIntoYear = timestamp.DayOfYear - 1;
        return new DateTime(timestamp.Year, 1, 1).AddDays(daysIntoYear - daysIntoYear % 7);
    }
}
