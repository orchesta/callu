...
/// <summary>Propagates a service's status along its dependency graph.</summary>
// BFS with a depth cap so an accidentally-deep chain cannot burn cycles, and monotonic worsening only — recovery is never
// inferred, an operator has to resolve the source incident.
    ...
    : IServiceStatusCascadeEngine
{
    ...
    /// <summary>Criticality-to-status derating matrix: how far an upstream status carries to a dependent.</summary>
