    ...
    ILogger<StatusPageService> logger) : IStatusPageService
{
    ...
    public async Task<StatusPageDto> CreateAsync(CreateStatusPageRequest request, CancellationToken cancellationToken = default)
    {
        return await transactionManager.ExecuteInTransactionAsync(async () =>
        {
            ...
            if (existing != null)
            {
                throw new ConflictException($"Status page with slug '{request.Slug}' already exists.");
            }
    ...
    public async Task<bool> TriggerSubscriberNotificationAsync(Guid incidentId, CancellationToken cancellationToken = default)
    {
        ...
        // Bus-outbox publishes only reach the broker when SaveChanges commits the outbox row;
        // without a transaction here the message is silently dropped in broker mode.
        await transactionManager.ExecuteInTransactionAsync(
            () => subscriberNotifier.NotifyAsync(incidentId, cancellationToken),
            cancellationToken);
        return true;
    }
    ...
    /// <summary>
    /// The uptime windows cached per status page, enumerated because HybridCache has no wildcard removal.
    /// </summary>
    ...
    internal static int SnapToUptimeBucket(int days)
    {
        var clamped = Math.Clamp(days, 7, 90);
        var nearest = CachedUptimeDays[0];
        foreach (var bucket in CachedUptimeDays)
        {
            if (Math.Abs(bucket - clamped) < Math.Abs(nearest - clamped))
                nearest = bucket;
        }
        return nearest;
    }
    ...
    public async Task<bool> SubscribeAsync(Guid pageId, string email, CancellationToken cancellationToken = default)
    {
        var page = await statusPageRepo.FindSingleAsync(
            p => p.Id == pageId && !p.IsDeleted && p.IsPublic && p.AllowSubscriptions, cancellationToken);
        if (page == null) return false;
    ...
    /// <summary>Trimmed projection for the anonymous public status-page endpoint.</summary>
    // Strips every health-check field that could leak credentials, internal topology or operational state; admin reads use MapToDetailDto.
