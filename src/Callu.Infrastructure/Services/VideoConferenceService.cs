    ...
    ILogger<VideoConferenceService> logger) : IVideoConferenceService
{
    public async Task<ConferenceRoomResult> CreateRoomAsync(Guid incidentId, CancellationToken ct = default)
    {
            ...
            (result, participants, incident, room) = await transactionManager.ExecuteInTransactionAsync(async () =>
        {
            ...
            if (incident.TeamId is { } incidentTeamId)
            {
                var teamMembers = await teamMemberRepo.GetQueryable()
                    .Where(tm => tm.TeamId == incidentTeamId && !tm.IsDeleted)
                    .ToListAsync(ct);
        ...
        // An existing active room was returned (room/incident null on that path) — its invites
        // were already sent when it was first created. Rooms stay active ~60 min, so re-triggering
        // a conference for the same incident within that window intentionally does not re-SMS.
        if (room == null || incident == null)
        {
            logger.LogInformation(
                "Conference for incident {IncidentId}: an active room already exists — no new SMS invite sent (invites go out only when the room is first created).",
                incidentId);
            return result;
        }

        // Room created but nobody to invite. Participants come from the incident's team; without a
        // team (or a team with no members) there are no SMS recipients. This is the most common
        // reason "no conference SMS was sent" — surface it instead of returning silently.
        if (participants.Count == 0)
        {
            logger.LogWarning(
                "Conference room {RoomId} for incident {IncidentId} has no participants to invite — the incident has no team assigned, or the team has no members. No SMS invite sent.",
                room.Id, incidentId);
            result.InvitesSentCount = 0;
            return result;
        }

        // Use the operator-configured public Base URL (Settings page), not the raw config — the
        // link is opened in a browser, so localhost/appsettings fallbacks produce dead links.
        var calluApiUrl = await organizationSettingsService.GetPublicBaseUrlAsync(ct);

        // The conference link is delivered by SMS (participants with a phone number) and by email
        // (participants with an email address, when SMTP is configured). Email addresses are looked
        // up from the same users — ConferenceParticipant only stores the phone number.
        var contacts = await userContacts.GetContactsByIdsAsync(
            participants.Select(p => p.UserId).Where(id => !string.IsNullOrEmpty(id)).ToList(), ct);
        var emailByUserId = contacts
            .Where(c => !string.IsNullOrEmpty(c.Email))
            .ToDictionary(c => c.Id, c => c.Email!, StringComparer.Ordinal);

        var smsProvider = providerRegistry.GetProvider(CommunicationCapability.Sms);
        if (smsProvider is null)
        {
            logger.LogWarning(
                "No SMS provider is enabled/assigned the SMS capability — conference SMS invites will be skipped. " +
                "Enable an SMS provider (e.g. Verimor) in Communications settings.");
        }

        var smtpConfigured = await emailService.IsConfiguredAsync(ct);
        if (!smtpConfigured)
        {
            logger.LogInformation(
                "SMTP is not configured — conference email invites will be skipped. Configure email in Settings to enable them.");
        }

        var uninvitable = participants.Count(p =>
            string.IsNullOrEmpty(p.PhoneNumber) && !emailByUserId.ContainsKey(p.UserId));
        if (uninvitable > 0)
        {
            logger.LogWarning(
                "Conference {RoomId}: {Count} of {Total} participant(s) have neither a phone number nor an email address and cannot be invited.",
                room.Id, uninvitable, participants.Count);
        }
        ...
        foreach (var participant in participants)
        {
            ...
            if (smsProvider is not null && !string.IsNullOrEmpty(participant.PhoneNumber))
            {
                var smsMessage =
                    Messages.Get("conferenceSms.title") + "\n" +
                    Messages.Get("conferenceSms.incident", ("title", incident.Title), ("severity", incident.Severity)) + "\n" +
                    Messages.Get("conferenceSms.join", ("link", conferenceLink)) + "\n" +
                    Messages.Get("conferenceSms.duration", ("minutes", room.MaxDurationMinutes));
                try
                {
                    var smsResult = await smsProvider.SendSmsAsync(new SendSmsRequest
                    {
            ...
            if (smtpConfigured && emailByUserId.TryGetValue(participant.UserId, out var email))
            {
                try
                {
                    // Same file-based template system as the other emails (Email/Templates/
                    // conference_invite.html) — customizable without recompilation.
                    var sent = await emailService.SendConferenceInviteAsync(
                        email, incident.Title, incident.Severity.ToString(), conferenceLink, room.MaxDurationMinutes, ct);
                    if (sent)
                    {
                        reached = true;
            ...
            if (reached) invitesSent++;
        }

        result.InvitesSentCount = invitesSent;
        return result;
    }
    ...
    public async Task<(IEnumerable<ConferenceRoomDto> Items, int TotalCount)> GetConferenceRoomsPagedAsync(ConferenceRoomFilter filter, CancellationToken ct = default)
    {
        ...
        if (filter.IncidentId is { } incidentId)
        {
    ...
    /// <summary>True when the exception wraps a Postgres unique violation on the given constraint or index.</summary>
    // Lets a create race (recoverable — return the winning room) be told from any other DB error; matches on SqlState as well as
    // ConstraintName, because an index declared without an explicit name surfaces its implementation-detail name there.
