    ...
    ILogger<VoximplantManagementService> logger) : IVoximplantManagementService
{
    ...
    public async Task<VoxCallHistoryDto> GetCallHistoryAsync(
        Guid providerId, VoxCallHistoryQuery query, CancellationToken cancellationToken = default)
    {
        var (fromUtc, toUtc, count) = NormalizeHistoryWindow(query.FromUtc, query.ToUtc, query.Count);
        var client = await CreateClientAsync(providerId, cancellationToken);
        var response = await client.GetCallHistoryAsync(
            fromUtc,
            toUtc,
            count,
            withCalls: true,
            remoteNumber: query.RemoteNumber,
            callSessionHistoryId: query.CallSessionHistoryId);

        ThrowIfError(response);

        return new VoxCallHistoryDto
        {
            TotalCount = response.TotalCount,
            Timezone = response.Timezone ?? "Etc/GMT",
    ...
    public async Task<VoxSessionLogDto> GetSessionLogAsync(
        Guid providerId, long callSessionHistoryId, CancellationToken cancellationToken = default)
    {
        if (callSessionHistoryId <= 0)
            throw new BusinessRuleException("A valid Voximplant session id is required.");

        var toUtc = DateTime.UtcNow.AddMinutes(5);
        var fromUtc = toUtc.AddDays(-31);
        var client = await CreateClientAsync(providerId, cancellationToken);
        var response = await client.GetCallHistoryAsync(
            fromUtc,
            toUtc,
        ...
        var session = response.Result?.FirstOrDefault();
        if (session is null)
            throw new NotFoundException("Voximplant session", callSessionHistoryId);

        if (string.IsNullOrWhiteSpace(session.LogFileUrl))
            throw new BusinessRuleException(
                "This session has no log file (logs are retained for about one month, then cleared).");

        const int maxBytes = 512_000;
        var (ok, body, error) = await client.DownloadUrlAsync(session.LogFileUrl, maxBytes);
        if (!ok || body is null)
            throw new BusinessRuleException($"Could not download the Voximplant session log: {error}");

        return new VoxSessionLogDto
        {
            CallSessionHistoryId = callSessionHistoryId,
            Content = body,
            Truncated = body.Length >= maxBytes - 64
        };
    }

    private static VoxCallSessionDto MapSession(VoxCallSessionInfoType s) => new()
    {
        CallSessionHistoryId = s.CallSessionHistoryId,
        ...
        HasLog = !string.IsNullOrWhiteSpace(s.LogFileUrl),
        Calls = (s.Calls ?? [])
            .Select(c => new VoxCallLegDto
            {
                CallId = c.CallId,
    ...
    private static (DateTime FromUtc, DateTime ToUtc, int Count) NormalizeHistoryWindow(
        DateTime? fromUtc, DateTime? toUtc, int count)
    {
        var to = (toUtc ?? DateTime.UtcNow).ToUniversalTime();
        var from = (fromUtc ?? to.AddHours(-24)).ToUniversalTime();
        if (from > to)
            throw new BusinessRuleException("fromUtc must be before toUtc.");
        if (to - from > TimeSpan.FromDays(7))
            throw new BusinessRuleException("Call history window cannot exceed 7 days.");

        return (from, to, Math.Clamp(count <= 0 ? 50 : count, 1, 100));
    }
