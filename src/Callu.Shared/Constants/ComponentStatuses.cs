...
public static class ComponentStatuses
{
    ...
    public static string FromServiceStatus(ServiceStatus status) => status switch
    {
        ServiceStatus.Operational => Operational,
        ServiceStatus.DegradedPerformance => Degraded,
        ServiceStatus.PartialOutage => PartialOutage,
        ServiceStatus.MajorOutage => MajorOutage,
        ServiceStatus.UnderMaintenance => Maintenance,
        _ => Operational
    };
    ...
    /// <summary>The single definition of a page-level status: the worst of its component statuses.</summary>
