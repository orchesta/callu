...
public class OrderedSingleRowReadGuardTests
{
    ...
    [
        ...
        new("IncidentEventDispatcher.cs", "var action = await serviceActions.GetQueryable() .FirstOrDefaultAsync(a => a.Id == actionId && !a.IsDeleted, cancellationToken)", Verdict.PrimaryKey),
        new("IncidentEventDispatcher.cs", "… GetQueryable() .Where(d => d.IncidentId == incidentId && d.AckType == chainKey) .OrderByDescending(d => d.AttemptedAt) .FirstOrDefaultAsync(cancellationToken)", Verdict.Ordered, "The double-click guard wants the newest attempt of this manual chain; ordering by AttemptedAt is what makes \"newest\" mean anything."),
        ...
        new("ServiceActionService.cs", "var action = await actions.GetQueryable() .FirstOrDefaultAsync(a => a.Id == actionId && a.ServiceId == serviceId && !a.IsDeleted, cancellationToken)", Verdict.PrimaryKey),
