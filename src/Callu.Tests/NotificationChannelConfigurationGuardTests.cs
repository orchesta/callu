...
public class NotificationChannelConfigurationGuardTests
{
    ...
    /// <summary>A channel with ONLY the closed (or reopened) trigger enabled is valid.</summary>
    [Fact]
    public void Accepts_ClosedOnly_Trigger()
    {
        NotificationChannelConfigurationGuard.EnsureValid(
            NotificationChannelType.Webhook,
            new Dictionary<string, string> { ["url"] = "https://1.1.1.1/hook" },
            notifyOnCreated: false, notifyOnAcknowledged: false, notifyOnResolved: false,
            notifyOnClosed: true, notifyOnReopened: false);
    }

    [Fact]
    public void Rejects_AllTriggersOff_IncludingClosedReopened()
    {
        Assert.Throws<ValidationException>(() =>
            NotificationChannelConfigurationGuard.EnsureValid(
                NotificationChannelType.Webhook,
                new Dictionary<string, string> { ["url"] = "https://1.1.1.1/hook" },
                notifyOnCreated: false, notifyOnAcknowledged: false, notifyOnResolved: false,
                notifyOnClosed: false, notifyOnReopened: false));
    }
}
