...
/// <summary>Integration-style coverage of on-call resolution over real repositories and a real HybridCache.</summary>
public class OnCallServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task Override_TakesPrimary_AndKeepsARosteredBackupAsSecondary()
    {
        var team = Guid.NewGuid();
        var sched = SeedSchedule(team);
        ...
        Assert.Equal("alice", status.SecondaryUserId);
    }

    [Fact]
    public async Task Override_OnTheRosteredPrimary_KeepsTheRosteredSecondaryAsBackup()
    {
        var team = Guid.NewGuid();
        var sched = SeedSchedule(team);
        SeedRoster(team, "alice", "bob");
        SeedOccurrence(sched, "alice", isPrimary: true, order: 1);
        SeedOccurrence(sched, "bob", isPrimary: false, order: 2);
        SeedOverride(sched, "alice", Now.Minus(Duration.FromHours(1)), Now.Plus(Duration.FromHours(2)));
        await SaveAsync();
    ...
    /// <summary>
    /// "Business hours" + "Out of hours" is the natural way to model a team, and at 03:00 the
    /// alphabetically-first one is empty.
    /// </summary>
    [Fact]
    public async Task ForTeam_FallsThroughToTheScheduleThatIsActuallyCovering()
    {
        var team = Guid.NewGuid();
        var businessHours = SeedSchedule(team, "Business hours");
        var outOfHours = SeedSchedule(team, "Out of hours");
        SeedRoster(team, "alice", "bob");
        SeedOccurrence(outOfHours, "bob", true, 1);   // only this one is covering right now
        await SaveAsync();

        var status = await _sut.GetCurrentOnCallForTeamAsync(team);

        Assert.Equal("bob", status!.PrimaryUserId);
        Assert.Equal(outOfHours, status.ScheduleId);
        Assert.NotEqual(businessHours, status.ScheduleId);
    }

    /// <summary>An earlier schedule that IS covering still wins — the fall-through is ordered, not a scan for anyone.</summary>
    [Fact]
    public async Task ForTeam_PrefersTheEarlierScheduleWhenBothAreCovering()
    {
        var team = Guid.NewGuid();
        var alpha = SeedSchedule(team, "Alpha");
        var beta = SeedSchedule(team, "Beta");
    ...
    /// <summary>
    /// A team whose every schedule is empty reports null rather than a schedule with a null
    /// responder, so the caller's "nobody on call" timeline row names the team.
    /// </summary>
    [Fact]
    public async Task ForTeam_NobodyCoveringAnySchedule_ReturnsNull()
    {
        var team = Guid.NewGuid();
        SeedSchedule(team, "Alpha");
        SeedSchedule(team, "Beta");
        SeedRoster(team, "alice");
        await SaveAsync();

        Assert.Null(await _sut.GetCurrentOnCallForTeamAsync(team));
    }
