...
public class VoximplantCallStatusTests
{
    ...
    /// <summary>Conference-room lifecycle callbacks are filtered before call-log handling, since they carry no session.</summary>
    [Theory]
    [InlineData("conference_started", true)]
    [InlineData("conference_ended", true)]
    [InlineData("participant_joined", true)]
    [InlineData("PARTICIPANT_LEFT", true)]
    [InlineData("conference_created", false)] // 9 handler — must keep acknowledging the incident
    [InlineData("connected", false)]
    [InlineData(null, false)]
    public void IsConferenceLifecycleStatus_FiltersRoomEventsOnly(string? status, bool expected) =>
        Assert.Equal(expected, VoximplantVoiceCallbackPersistence.IsConferenceLifecycleStatus(status));
