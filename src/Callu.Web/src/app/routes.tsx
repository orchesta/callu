...
import { PERMISSIONS, type Permission } from "@/shared/auth/roles";
...
const IncidentAuditTrail = lazy(() => import("@/features/audit-logs/components/incident-trail").then(m => ({ default: m.IncidentAuditTrail })));
const TraceExplorer = lazy(() => import("@/features/diagnostics/components/trace-explorer").then(m => ({ default: m.TraceExplorer })));
...
/**
 * Lazy route behind a claim check — redirects to /auth/access-denied when the
 * signed-in role lacks the permission the API requires for that page.
 */
...
export const router = createBrowserRouter([
  ...
  },
  {
    ...
    children: [
      ...
      {
        // The create form is a dialog on the list, so this path opens it rather than 404ing —
        // it was a bookmarkable URL before there was a form behind it.
        path: "incidents/new",
        loader: () => redirect('/incidents?create=1'),
      },
      {
        path: "incidents/:id",
      ...
      {
        path: "users",
        Component: () => <GuardedLazyRoute Component={UsersPage} permission={PERMISSIONS.ManageUsers} />,
      },
      {
        path: "services",
      ...
      {
        path: "audit-logs",
        Component: () => <GuardedLazyRoute Component={AuditLogList} permission={PERMISSIONS.ViewAuditLog} />,
      },
      {
        path: "audit-logs/incident/:id",
        Component: () => <GuardedLazyRoute Component={IncidentAuditTrail} permission={PERMISSIONS.ViewAuditLog} />,
      },
      {
        path: "diagnostics",
        Component: () => <GuardedLazyRoute Component={TraceExplorer} permission={PERMISSIONS.ManageSettings} />,
      },
      {
        path: "reports",
      ...
      {
        path: "maintenance",
        Component: () => <GuardedLazyRoute Component={MaintenancePage} permission={PERMISSIONS.ManageSettings} />,
      },
      {
        path: "settings",
        Component: () => <GuardedLazyRoute Component={SettingsHub} permission={PERMISSIONS.ManageSettings} />,
      },
      {
        path: "settings/communications",
        Component: () => <GuardedLazyRoute Component={CommunicationsHub} permission={PERMISSIONS.ManageSettings} />,
      },
      {
        path: "settings/communications/voximplant/:id",
        Component: () => <GuardedLazyRoute Component={VoximplantManagement} permission={PERMISSIONS.ManageSettings} />,
      },
      {
        path: "settings/email-templates",
        Component: () => <GuardedLazyRoute Component={EmailTemplates} permission={PERMISSIONS.ManageSettings} />,
      },
      {
        path: "settings/status-page",
        Component: () => <GuardedLazyRoute Component={StatusPageManagement} permission={PERMISSIONS.ManageSettings} />,
      },
      {
        path: "*",
