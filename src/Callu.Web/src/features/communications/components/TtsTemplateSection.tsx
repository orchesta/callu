/** TTS template list plus create/edit modal; message keys are loaded from the backend and grouped by category. */

import { useState, useEffect, useCallback, useId } from "react";
import { toast } from "@/shared/utils/toast";
import { t } from "@/shared/locales/i18n";
import { usePreviewTtsTemplate } from "../../settings/hooks/use-communications";
import { RenderedSegment } from "./TtsPreviewSection";
...
import type { TtsPreviewSegment } from "../../settings/types/communications.types";
...
export function TtsTemplateSection({
    ...
    const [rendered, setRendered] = useState<{ languageCode: string; segments: TtsPreviewSegment[] } | null>(null);
    const previewTemplate = usePreviewTtsTemplate();

    const handlePreview = async (languageCode: string) => {
        setRendered(null);
        try {
            setRendered({ languageCode, segments: await previewTemplate.mutateAsync(languageCode) });
        } catch {
            // useApiMutation surfaces the reason; leaving a stale render on screen would read as this one.
        }
    };
    ...
    const handleLanguageChange = (value: string) => {
        // Display name gates the submit, so leaving it blank here means Load Defaults fills every
        // message and the button still refuses. Only filled when untouched.
        const suggested = languageOptions
            .find((lang) => lang.value === value)?.label
            .replace(/^\P{L}+/u, "")
            .trim();
        ...
        setFormData({
            ...formData,
            languageCode: value,
            displayName: formData.displayName.trim().length > 0 ? formData.displayName : suggested ?? "",
        });
        if (!editingTts) {
            loadDefaults(value);
        }
                ...
                {ttsTemplates.length > 0 ? (
                    <div className="space-y-3">
                        {ttsTemplates.map((template) => (
                            <div
                                key={template.id}
                                className="p-3 rounded-lg border border-border bg-surface-light/20 hover:border-border-light transition-all flex items-center gap-4"
                            >
                                ...
                                <div className="flex gap-2">
                                    <Button
                                        ...
                                        onClick={() => handlePreview(template.languageCode)}
                                        aria-label={t("tts.preview.playTemplate")}
                                        title={t("tts.preview.playTemplate")}
                                    >
                    ...
                    {rendered && rendered.segments.length > 0 && (
                        <div className="mt-4 space-y-2 rounded-lg border border-border p-3">
                            <p style={{ fontSize: "0.8125rem", color: "#94A3B8" }}>
                                {t("tts.preview.whatACallWouldSay", { language: rendered.languageCode })}
                            </p>
                            {/* The spoken text, not the template: this is where a wrong language shows. */}
            ...
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                <DialogContent className="bg-card border-border sm:max-w-[750px] max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="space-y-5 py-4">
                        <div>
                            <label htmlFor={`${formId}-language`} style={{ fontSize: "0.875rem", fontWeight: 600, marginBottom: "0.5rem", display: "block" }}>
                            ...
                            <Select
                                ...
                                disabled={!!editingTts}
                            >
                                <SelectTrigger id={`${formId}-language`} className="bg-input-background">
                        ...
                        <div>
                            <label style={{ fontSize: "0.875rem", fontWeight: 600, marginBottom: "0.5rem", display: "block" }}>
                                {t("tts.displayName")} <span style={{ color: "#EF4444" }}>*</span>
                            </label>
                            <Input
