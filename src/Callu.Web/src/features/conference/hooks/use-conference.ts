...
/**
 * Create a conference room for an incident. This is the only path that sends the SMS
 * invite (with the join link) to the incident team's members who have a phone number.
 */
export function useCreateConferenceRoom() {
    return useApiMutation(
        (incidentId: string) => conferenceApi.createRoom(incidentId),
        { successMessage: t('incidents.conferenceStarted') },
    );
}
