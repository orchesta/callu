...
import { Server, Clock, ChevronRight, Filter, CheckCircle, XCircle, LayoutGrid, Table2, Plus } from "lucide-react";
...
export function IncidentsList() {
  ...
  // The filters live in the URL so a filtered view can be linked to, survives a reload, and can be
  // stepped back out of. The text box keeps its own copy because it has to stay responsive per
  // keystroke; the URL only catches up once the value settles.
  ...
  const { user } = useAuth();
  const canCreate = canManageIncidents(user?.role);

  const openCreate = useCallback(() => setIsCreateOpen(true), []);
  const closeCreate = useCallback(
    (open: boolean) => {
      setIsCreateOpen(open);
      if (open || !searchParams.has("create")) return;
  ...
  // Debounced into the key rather than taken straight from the input: `searchQuery` reaches an
  // unindexed scan over every incident, on the same database the paging path is using.
  ...
  const filter: IncidentFilter = useMemo(
    ...
    [statusFilter, severityFilter, debouncedSearch, page, pageSize],
  );
  ...
  // Any filter change restarts paging: page 3 of the old filter is not page 3 of the new one.
  ...
  // Replaces rather than pushes: one history entry per keystroke would make Back unusable.
  useEffect(() => {
  ...
  // A shared link outlives the page count it was made against. Left alone the operator gets an empty
  // list reading "no incidents found", which is a different statement from "that page is gone".
  useEffect(() => {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {isCreateOpen && <CreateIncidentDialog open onOpenChange={closeCreate} />}
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <Select value={statusFilter} onValueChange={(v) => setFilter("status", v)}>
          <SelectTrigger
            className="w-full sm:w-[180px] bg-input-background backdrop-blur-sm"
        ...
        >
          <SelectTrigger
            className="w-full sm:w-[180px] bg-input-background backdrop-blur-sm"
            aria-label={t("incidents.severity")}
          >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2 order-1 sm:order-2">
          ...
          >
            <SelectTrigger
              className="w-full sm:w-[130px] h-9 bg-input-background backdrop-blur-sm"
              aria-label={t("incidents.perPage")}
            >
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          ...
          onClick={() =>
            setFilter("status", statusFilter === "Open" ? "all" : "Open")
          }
          className={
            statusFilter === "Open"
        ...
        <Button
          ...
          onClick={() =>
            setFilter("severity", severityFilter === "Critical" ? "all" : "Critical")
          }
          className={
        ...
        <Button
          ...
          onClick={() =>
            setFilter("severity", severityFilter === "High" ? "all" : "High")
          }
          className={
            severityFilter === "High"
        ...
        {incidents.length > 0 && viewMode === "cards" && (
          <div className="flex items-center gap-3 px-4 py-2 border-b border-border bg-muted/30">
            <Checkbox
              ...
              aria-label={t("incidents.selectAll")}
