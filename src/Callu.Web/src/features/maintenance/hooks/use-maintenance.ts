...
export function useUpdateMaintenanceWindow() {
  const qc = useQueryClient();
  return useApiMutation<MaintenanceWindowDto, { id: string } & UpdateMaintenanceWindowRequest>(
    ({ id, ...data }) => maintenanceApi.update(id, data),
    {
      successMessage: "Maintenance window updated",
      onSuccess: () => {
        qc.invalidateQueries({ queryKey: MW_KEY });
        qc.invalidateQueries({ queryKey: MW_ACTIVE_KEY });
      },
