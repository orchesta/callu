...
export const notificationKeys = {
    ...
    /** `count` is part of the key: the header bell and the notifications page ask for different sizes. */
    recent: (count: number) => [...notificationKeys.all, 'recent', count] as const,
    /** Prefix for invalidation: every `count` variant at once. */
    recentAll: () => [...notificationKeys.all, 'recent'] as const,
    unreadCount: () => [...notificationKeys.all, 'unread-count'] as const,
};

export const notificationQueries = {
    recent: (count = 20) =>
        apiQueryOptions(notificationKeys.recent(count), () => notificationApi.getRecent(count), { staleTime: 30_000 }),
    unreadCount: () =>
