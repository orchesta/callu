...
import { formatMinutes } from "@/shared/utils/duration";
import {
    useIncidentTrends,
    useMttMetrics,
    useServiceUptime,
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    <Select value={groupBy} onValueChange={setGroupBy}>
                        <SelectTrigger className="bg-input-background w-[120px]" aria-label={t("reports.groupBy")}>
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <Select value={range} onValueChange={setRange}>
                        <SelectTrigger className="bg-input-background w-[160px]" aria-label={t("reports.dateRange")}>
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                            {PRESET_RANGES.map((r) => (
                        ...
                        {!trends || trends.length === 0 ? (
                            ...
                            <div className="space-y-2">
                                            ...
                                            >
                                                ...
                                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-10">
                                                    <div className="bg-popover border border-border rounded-lg p-2 shadow-lg whitespace-nowrap text-xs">
                                                        <p style={{ fontWeight: 600 }}>{new Date(point.date).toLocaleDateString(dateLocale())}</p>
                                ...
                                <div className="flex justify-between text-xs" style={{ color: "#64748B" }}>
                                    <span>{trends.length > 0 ? new Date(trends[0].date).toLocaleDateString(dateLocale()) : ""}</span>
                                    <span>{trends.length > 0 ? new Date(trends[trends.length - 1].date).toLocaleDateString(dateLocale()) : ""}</span>
                                </div>
                            </div>
                        )}
                    </Card>
