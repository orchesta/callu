...
export function RunbooksList() {
    ...
    const [viewing, setViewing] = useState<RunbookDto | null>(null);
    const [deleting, setDeleting] = useState<RunbookDto | null>(null);
    ...
    // /runbooks/:id (deep link, e.g. from incident detail): open the viewer and record usage once.
    const usageMarkedRef = useRef(false);
    useEffect(() => {
        if (!routeId || !runbooks || isEditorOpen) return;
        const rb = runbooks.find((r) => r.id === routeId);
        if (!rb) {
            toast.error(t("runbooks.notFound"));
            navigate("/runbooks", { replace: true });
            return;
        }
        setViewing(rb);
        if (!usageMarkedRef.current) {
            usageMarkedRef.current = true;
            runbooksApi.markUsed(rb.id)
                .then(() => qc.invalidateQueries({ queryKey: runbookKeys.all }))
                .catch(() => { /* usage tracking must never block viewing */ });
        }
    ...
    const closeViewer = (open: boolean) => {
        if (!open) {
            setViewing(null);
            if (routeId) navigate("/runbooks", { replace: true });
        }
    };
    ...
    return (
        <div className="p-6 space-y-6">
                                ...
                                <div className="flex items-center gap-3 text-xs" style={{ color: "#64748B" }}>
                                    ...
                                    {rb.lastUsedAt && (
                                        <span className="flex items-center gap-1">
                                            ...
                                            {new Date(rb.lastUsedAt).toLocaleDateString(dateLocale())}
                                        </span>
                                    )}
                                </div>
                                <div className="flex items-center gap-1">
                                    <Button size="sm" variant="outline" title={t("common.view")} onClick={() => setViewing(rb)}>
                                        <Eye className="w-3 h-3" />
                                    </Button>
                                    <Button
                                        size="sm"
                                        variant="outline"
                                        aria-label={t("runbooks.markUsedAria").replace("{title}", rb.title)}
                                        onClick={() => markUsedMutation.mutate(rb.id)}
                                    >
                                        <Play className="w-3 h-3" />
                                    </Button>
                                    <Button size="sm" variant="outline" title={t("common.edit")} onClick={() => openEdit(rb)}>
                                    ...
                                    <Button
                                        ...
                                        aria-label={t("runbooks.deleteAria").replace("{title}", rb.title)}
                ...
                <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
                    ...
                    {viewing && (
                        <div className="space-y-4 py-2">
                            {viewing.description && (
                                <p style={{ fontSize: "0.875rem", color: "#94A3B8" }}>{viewing.description}</p>
                            )}
                            {viewing.serviceName && (
                                <p style={{ fontSize: "0.75rem", color: "#64748B" }}>
                                    {t("runbooks.service")}: {viewing.serviceName}
                                </p>
                            )}
                            ...
                            <pre className="p-3 rounded-lg bg-input-background border border-border text-sm whitespace-pre-wrap font-mono">{viewing.content}</pre>
                        </div>
                    )}
                    <DialogFooter>
                        <Button variant="outline" onClick={() => { const rb = viewing; setViewing(null); if (rb) openEdit(rb); }}>
            ...
            <DeleteConfirmDialog
                ...
                message={t("runbooks.deleteMsg").replace("{title}", deleting?.title ?? "")}
