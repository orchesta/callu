...
export const runbookKeys = {
    ...
    byService: (serviceId: string) => [...runbookKeys.all, 'by-service', serviceId] as const,
};
...
export const runbookQueries = {
    ...
    byService: (serviceId: string) =>
        apiQueryOptions<RunbookDto[]>(
            runbookKeys.byService(serviceId),
            () => runbooksApi.getByService(serviceId),
            { enabled: !!serviceId },
        ),
};
...
export function useRunbooksByService(serviceId: string) {
    return useQuery(runbookQueries.byService(serviceId));
}
