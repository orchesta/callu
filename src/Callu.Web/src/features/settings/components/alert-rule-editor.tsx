...
import { useFieldArray, type UseFormRegisterReturn } from "react-hook-form";
...
interface AlertRuleEditorProps {
  ...
  /** Named inbound webhook endpoints (API: integrations). */
  inboundWebhooks?: Array<{ id: string; name: string }>;
                ...
                return (
                  <div key={field.id} className="flex flex-wrap items-center gap-2">
                    <Select
                      ...
                      onValueChange={(v) => {
                        form.setValue(`conditions.${i}.field`, v);
                        form.setValue(`conditions.${i}.value`, "");
                      }}
                    >
                      <SelectTrigger
                        className="bg-input-background w-full sm:w-[140px]"
                        aria-label={t("alertRules.ariaConditionField", { index: i + 1 })}
                      >
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    >
                      <SelectTrigger
                        className="bg-input-background w-full sm:w-[130px]"
                        aria-label={t("alertRules.ariaConditionOperator", { index: i + 1 })}
                      >
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <ConditionValueInput
                      ...
                      onChange={(v) => form.setValue(`conditions.${i}.value`, v)}
                      register={form.register(`conditions.${i}.value`)}
                    ...
                    <Button
                      ...
                      size="sm"
                      variant="ghost"
                      aria-label={t("alertRules.ariaRemoveCondition", { index: i + 1 })}
                      onClick={() => conditionsField.remove(i)}
                    >
              ...
              {actionsField.fields.map((field, i) => {
                ...
                return (
                  <div key={field.id} className="flex flex-wrap items-center gap-2">
                    ...
                    >
                      <SelectTrigger
                        className="bg-input-background w-full sm:w-[180px]"
                        aria-label={t("alertRules.ariaActionType", { index: i + 1 })}
                      >
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    {actionType === "SetSeverity" ? (
                      ...
                      >
                        <SelectTrigger
                          className="bg-input-background min-w-[12rem] flex-1"
                          aria-label={t("alertRules.ariaActionValue", { index: i + 1 })}
                        >
                      ...
                      <div style={{ flex: 1 }} className="space-y-1.5">
                        <span style={{ fontSize: "0.8125rem", color: "#94A3B8", display: "block" }}>
                          {t("alertRules.suppressNotificationHint")}
                        </span>
                        <label className="flex items-center gap-2 text-sm cursor-pointer">
                          <input
                            type="checkbox"
                            checked={form.watch(`actions.${i}.suppressPaging`) ?? false}
                            onChange={(e) => form.setValue(`actions.${i}.suppressPaging`, e.target.checked)}
                            className="h-4 w-4 accent-brand-500"
                          />
                          <span>{t("alertRules.suppressPagingLabel")}</span>
                        </label>
                        {form.watch(`actions.${i}.suppressPaging`) && (
                          <span style={{ fontSize: "0.75rem", color: "#f59e0b", display: "block" }}>
                            {t("alertRules.suppressPagingWarning")}
                          </span>
                        )}
                      </div>
                    ) : (
                      ...
                      >
                        <SelectTrigger
                          className="bg-input-background min-w-[12rem] flex-1"
                          aria-label={t("alertRules.ariaActionTarget", { index: i + 1 })}
                        >
                    ...
                    <Button
                      ...
                      size="sm"
                      variant="ghost"
                      aria-label={t("alertRules.ariaRemoveAction", { index: i + 1 })}
                      onClick={() => actionsField.remove(i)}
                    >
                      <X className="w-4 h-4 text-muted-foreground" />
                    </Button>
                  </div>
...
function ConditionValueInput({
  ...
  const aria = t("alertRules.ariaConditionValue", { index: index + 1 });

  if (fieldValue === "Severity" || fieldValue === "Status") {
    const options = fieldValue === "Severity"
      ? severityValues
      : ["Open", "Acknowledged", "Resolved"];
    return (
      <Select value={value} onValueChange={onChange}>
        <SelectTrigger className="bg-input-background min-w-[12rem] flex-1" aria-label={aria}>
  ...
  if (fieldValue === "Service") {
    return (
      <EntitySelect
        ...
        onChange={onChange}
        options={services}
        emptyLabel={t("alertRules.noServices")}
        orphanLabel={t("alertRules.unknownService")}
      />
    );
  }
  ...
  if (fieldValue === "Team") {
    return (
      <EntitySelect
        ...
        onChange={onChange}
        options={teams}
        emptyLabel={t("alertRules.noTeams")}
        orphanLabel={t("alertRules.unknownTeam")}
      />
    );
  }
  ...
  if (fieldValue === "Source") {
    return (
      <EntitySelect
        ...
        value={value}
        onChange={onChange}
        options={inboundWebhooks}
        emptyLabel={t("alertRules.noInboundWebhooks")}
        orphanLabel={t("alertRules.unknownInboundWebhook")}
      />
    );
  }
    ...
    <Select value={value || undefined} onValueChange={onChange}>
      ...
      <SelectContent>
        {options.length === 0 && !value && (
          <SelectItem value="__empty__" disabled>{emptyLabel}</SelectItem>
        )}
        {value && !known && (
          <SelectItem value={value}>{orphanLabel}</SelectItem>
        )}
        {options.map((o) => (
