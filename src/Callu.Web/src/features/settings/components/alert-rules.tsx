...
export function AlertRulesSettings() {
  ...
  const { data: inboundWebhooks } = useIntegrations();
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-xs" style={{ color: "#64748B" }}>
                      <span>{t("alertRules.conditionCount", { count: String(rule.conditions.length) })}</span>
                      <span>&middot;</span>
                      <span>{t("alertRules.actionCount", { count: String(rule.actions.length) })}</span>
                      <span>&middot;</span>
                      <span>{t("alertRules.triggeredCount", { count: String(rule.triggerCount) })}</span>
                      {rule.lastTriggeredAt && (
                        <>
                          <span>&middot;</span>
                          <span>{t("alertRules.lastTriggered", { time: new Date(rule.lastTriggeredAt).toLocaleString(dateLocale()) })}</span>
                        </>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <Button
                      ...
                      aria-label={t(rule.isEnabled ? "alertRules.ariaDisableRule" : "alertRules.ariaEnableRule", { name: rule.name })}
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      aria-label={t("alertRules.ariaEditRule", { name: rule.name })}
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      aria-label={t("alertRules.ariaDeleteRule", { name: rule.name })}
                      onClick={() => {
      ...
      <AlertRuleEditor
        ...
        inboundWebhooks={(inboundWebhooks ?? []).map((w) => ({ id: w.id, name: w.name }))}
        onSave={handleSave}
