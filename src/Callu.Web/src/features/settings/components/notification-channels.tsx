...
export function NotificationChannelsSettings() {
    ...
    const [historyChannel, setHistoryChannel] = useState<NotificationChannelDto | null>(null);
    const [deletingChannel, setDeletingChannel] = useState<NotificationChannelDto | null>(null);
    const { data: history, isLoading: historyLoading } = useChannelDeliveries(historyChannel?.id ?? null);
    ...
    const [form, setForm] = useState<CreateNotificationChannelRequest>({
        ...
        notifyOnIncidentClosed: false,
        notifyOnIncidentReopened: false,
    });
    ...
    const openEdit = (ch: NotificationChannelDto) => {
        ...
        setForm({
            ...
            notifyOnIncidentClosed: ch.notifyOnIncidentClosed ?? false,
            notifyOnIncidentReopened: ch.notifyOnIncidentReopened ?? false,
        });
        setIsEditorOpen(true);
    };
            ...
            if (editing) {
                await updateMutation.mutateAsync({
                    ...
                    data: {
                        ...
                        notifyOnIncidentClosed: form.notifyOnIncidentClosed,
                        notifyOnIncidentReopened: form.notifyOnIncidentReopened,
                    },
                });
            } else {
    ...
    const lifecycleAny =
        ...
        form.notifyOnIncidentClosed ||
        form.notifyOnIncidentReopened;
    ...
    return (
        <div className="space-y-6">
                                ...
                                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                    <div className="flex items-start gap-3 min-w-0">
                                        ...
                                        <div className="min-w-0 space-y-2">
                                            <div className="flex flex-wrap items-center gap-2">
                                                ...
                                                {ch.notifyOnIncidentClosed && (
                                                    <Badge variant="outline" className="text-[10px] font-normal">
                                                        {t("notificationChannels.badgeClosed")}
                                                    </Badge>
                                                )}
                                                {ch.notifyOnIncidentReopened && (
                                                    <Badge variant="outline" className="text-[10px] font-normal">
                                                        {t("notificationChannels.badgeReopened")}
                                                    </Badge>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                                                ...
                                                {ch.lastNotifiedAt && (
                                                    <span>
                                                        ...
                                                        {new Date(ch.lastNotifiedAt).toLocaleString(dateLocale())}
                                                    </span>
                                                )}
                                                {ch.lastDeliveryStatus && (
                                                    ...
                                                    >
                                                        <Badge
                                                            className={`border text-[10px] font-normal ${DELIVERY_STYLE[ch.lastDeliveryStatus]}`}
                                                        >
                                                            {t(`notificationChannels.delivery${ch.lastDeliveryStatus}`)}
                                                        </Badge>
                                                        <span>{t("notificationChannels.viewHistory")}</span>
                                                    </button>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        <Switch
                                            ...
                                            aria-label={t(ch.isEnabled ? "notificationChannels.ariaDisable" : "notificationChannels.ariaEnable").replace("{name}", ch.name)}
                                            onCheckedChange={() => toggleMutation.mutate(ch.id)}
                                        />
                                        <Button
                                            size="sm"
                                            variant="outline"
                                            aria-label={t("notificationChannels.testChannelAria").replace("{name}", ch.name)}
                                        ...
                                        <Button
                                            size="sm"
                                            variant="outline"
                                            aria-label={t("notificationChannels.editChannelAria").replace("{name}", ch.name)}
                                            onClick={() => openEdit(ch)}
                                        >
                                        ...
                                        <Button
                                            ...
                                            aria-label={t("notificationChannels.deleteChannelAria").replace("{name}", ch.name)}
            ...
            >
                <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col p-0 gap-0 overflow-hidden">
                    <DialogHeader className="p-6 pb-4 shrink-0 border-b border-border">
                        ...
                        {selectedType?.notice && (
                            <Alert
                                variant={selectedType.notice.level === "warning" ? "destructive" : "default"}
                                className="mt-3 text-left"
                            >
                                <AlertCircle />
                                <AlertDescription>{selectedType.notice.text}</AlertDescription>
                            </Alert>
                        )}
                    </DialogHeader>
                    ...
                    <ScrollArea className="flex-1 min-h-0 max-h-[calc(90vh-12rem)]">
                        <div className="p-6 space-y-6">
                            ...
                            {!editing && (
                                <div className="space-y-2">
                                    <label htmlFor={`${formId}-channel-type`} className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                                    ...
                                    >
                                        <SelectTrigger id={`${formId}-channel-type`}>
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {channelTypes.map((ct) => (
                                            ...
                                            {typeFields.map((field) => (
                                                <div key={field.key}>
                                                    <div className="flex items-center justify-between gap-2">
                                                        <label
                                                            htmlFor={`${formId}-config-${field.key}`}
                                        ...
                                        <div className="mt-4 space-y-2 rounded-md border border-border bg-muted/15 p-3">
                                            <div className="flex flex-wrap items-center justify-between gap-2">
                                                ...
                                                <Button
                                                    ...
                                                    onClick={async () => {
                                                        try {
                                                            await navigator.clipboard.writeText(selectedType.samplePayload ?? "");
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                {t("notificationChannels.samplePayloadHint")}
                                            </p>
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-3 rounded-md border border-border bg-muted/10 p-3">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <label className="flex items-center justify-between gap-3 text-sm cursor-pointer">
                                                <span>{t("notificationChannels.triggerClosed")}</span>
                                                <Switch
                                                    checked={form.notifyOnIncidentClosed}
                                                    onCheckedChange={(v) =>
                                                        setForm((f) => ({ ...f, notifyOnIncidentClosed: Boolean(v) }))
                                                    }
                                                />
                                            </label>
                                            <label className="flex items-center justify-between gap-3 text-sm cursor-pointer">
                                                <span>{t("notificationChannels.triggerReopened")}</span>
                                                <Switch
                                                    checked={form.notifyOnIncidentReopened}
                                                    onCheckedChange={(v) =>
                                                        setForm((f) => ({ ...f, notifyOnIncidentReopened: Boolean(v) }))
                                                    }
                                                />
                                            </label>
                                        </div>
                                    ...
                                    <div className="space-y-2">
                                        <label htmlFor={`${formId}-min-severity`} className="text-sm font-medium">{t("notificationChannels.minimumSeverity")}</label>
                                        <Select
                                        ...
                                        >
                                            <SelectTrigger id={`${formId}-min-severity`}>
            ...
            <Dialog open={!!historyChannel} onOpenChange={(open) => !open && setHistoryChannel(null)}>
                <DialogContent className="sm:max-w-2xl max-h-[85vh] flex flex-col">
                    <DialogHeader>
                        <DialogTitle>{t("notificationChannels.historyTitle")}</DialogTitle>
                        <DialogDescription className="text-left">
                            {historyChannel?.name} — {t("notificationChannels.historyHint")}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <ScrollArea className="flex-1 min-h-0 max-h-[60vh]">
                        ...
                        {!historyLoading && (history?.items.length ?? 0) === 0 && (
                            <p className="p-6 text-sm text-muted-foreground text-center">
                                {t("notificationChannels.historyEmpty")}
                            </p>
                        )}
                        <div className="space-y-2 pr-3">
                            {history?.items.map((d) => (
                                <div key={d.id} className="rounded-md border border-border bg-muted/10 p-3 space-y-1">
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                        <span className="text-sm font-medium">{d.title}</span>
                                        <Badge className={`border text-[10px] font-normal ${DELIVERY_STYLE[d.status]}`}>
                                            {t(`notificationChannels.delivery${d.status}`)}
                                        </Badge>
                                    </div>
                                    <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                                        <span>{d.eventKey}</span>
                                        <span>{new Date(d.attemptedAt).toLocaleString(dateLocale())}</span>
                                        <span>
                                            {t("notificationChannels.attemptNo", { count: String(d.attemptCount) })}
                                        </span>
                                    ...
                                    {d.messageText && (
                                        <div className="space-y-1">
                                            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
                                                {t("notificationChannels.messageSent")}
                                            </p>
                                            <pre className="max-h-32 overflow-y-auto rounded bg-muted/40 p-2 text-xs whitespace-pre-wrap break-words font-sans">
                                                {d.messageText}
                                            </pre>
                                        </div>
                                    )}
                                    {d.error && (
                                        <p className="text-xs font-mono text-error-400 break-all">{d.error}</p>
                                    )}
                                    {d.nextRetryAt && (
                                        <p className="text-xs text-muted-foreground">
                                            {t("notificationChannels.nextRetry")}{" "}
                                            {new Date(d.nextRetryAt).toLocaleString(dateLocale())}
                                        </p>
                                    )}
                                </div>
                            ))}
                    ...
                    {(history?.totalCount ?? 0) > (history?.items.length ?? 0) && (
                        <p className="text-xs text-muted-foreground">
                            {t("notificationChannels.historyTruncated", {
                                shown: String(history?.items.length ?? 0),
                                total: String(history?.totalCount ?? 0),
                            })}
                        </p>
                    )}
                </DialogContent>
            ...
            <DeleteConfirmDialog
                ...
                message={t("notificationChannels.deleteMsg").replace("{name}", deletingChannel?.name ?? "")}
                warning={t("notificationChannels.deleteWarn")}
                isLoading={deleteMutation.isPending}
                onConfirm={() => {
                    if (!deletingChannel) return;
                    deleteMutation.mutate(deletingChannel.id, { onSettled: () => setDeletingChannel(null) });
                }}
            />
        </div>
    );
