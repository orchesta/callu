...
export const communicationKeys = {
    ...
    capabilityRoutes: () => [...communicationKeys.all, 'capability-routes'] as const,
...
export const communicationQueries = {
    ...
    capabilityRoutes: () =>
        apiQueryOptions(communicationKeys.capabilityRoutes(), () => communicationsApi.getCapabilityRoutes(), {
            staleTime: 60_000,
        }),
    sipTrunks: () =>
...
export function useCapabilityRoutes() {
    return useQuery(communicationQueries.capabilityRoutes());
}

export function useSetCapabilityRoute() {
    const qc = useQueryClient();
    return useApiMutation(
        ({ capability, providerId }: { capability: string; providerId: string | null }) =>
            communicationsApi.setCapabilityRoute(capability, providerId),
        {
            // Which channel a provider owns changes what the providers list means, so both refresh.
            onSuccess: () => {
                qc.invalidateQueries({ queryKey: communicationKeys.capabilityRoutes() });
...
/** Renders sample text through the voice service. Nothing is cached: the point is to hear this text, now. */
export function usePreviewTts() {
    return useApiMutation((data: TtsPreviewRequest) => communicationsApi.previewTts(data));
}

/** Renders one saved template the way a call would speak it. */
export function usePreviewTtsTemplate() {
    return useApiMutation((languageCode: string) => communicationsApi.previewTtsTemplate(languageCode));
}
