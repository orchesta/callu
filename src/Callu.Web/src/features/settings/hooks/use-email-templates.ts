...
/** System templates are refused by the server; the screen does not offer it for them. */
export function useDeleteEmailTemplate() {
    const qc = useQueryClient();
    return useApiMutation(
        (id: string) => emailTemplateApi.delete(id),
        {
            // Drop the detail query rather than invalidating it: the template is gone, and a
            // refetch of its id answers 404.
            onSuccess: (_, id) => {
