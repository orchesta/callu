...
export function useChannelDeliveries(channelId: string | null) {
    return useQuery({
        ...apiQueryOptions<PagedDeliveries>(
            ['notification-channels', channelId ?? 'none', 'deliveries'],
            () => notificationChannelsApi.getDeliveries(channelId!),
            { staleTime: 30_000 },
        ),
        enabled: !!channelId,
    });
