...
export const settingsKeys = {
    ...
    firebase: () => [...settingsKeys.all, 'firebase'] as const,
...
export const settingsQueries = {
    ...
    firebase: () =>
        apiQueryOptions(settingsKeys.firebase(), () => settingsApi.getFirebase(), { staleTime: 5 * 60_000 }),
...
export function useFirebaseSettings() {
    return useQuery(settingsQueries.firebase());
}

export function useSaveFirebase() {
    const qc = useQueryClient();
    return useApiMutation(
        (data: UpdateFirebaseSettingsRequest) => settingsApi.saveFirebase(data),
        {
            onSuccess: () =>
                qc.invalidateQueries({ queryKey: settingsKeys.firebase() }),
        },
    );
}

export function useTestFirebase() {
    return useApiMutation(
        () => settingsApi.testFirebase(),
    );
}
