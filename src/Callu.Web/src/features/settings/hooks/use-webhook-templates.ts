...
/** Every saved template, built-in and custom. */
...
/** Deleting a template also detaches it from every service that was using it. */
export function useDeleteWebhookTemplate() {
    ...
    return useApiMutation(
        ...
        {
            onSuccess: (_, id) => {
                qc.removeQueries({ queryKey: webhookTemplateKeys.detail(id) });
                qc.invalidateQueries({ queryKey: webhookTemplateKeys.lists() });
...
/** Runs a template against a sample payload without touching anything. */
