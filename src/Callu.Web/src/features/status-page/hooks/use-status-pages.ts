...
/** Counts one visit to a public page. Fire-and-forget: a visitor must never see it fail. */
export function useRecordStatusPageView(pageId: string | undefined) {
    const counted = useRef<string | undefined>(undefined);

    useEffect(() => {
        if (!pageId || counted.current === pageId) return;
        counted.current = pageId;
        void statusPageApi.recordView(pageId).catch(() => { /* the page is what matters */ });
    }, [pageId]);
}

/** Removes the page and everything hanging off it: components, incident history, subscribers. */
export function useDeleteStatusPage() {
    ...
    return useApiMutation(
        ...
        {
            onSuccess: (_, id) => {
                qc.removeQueries({ queryKey: statusPageKeys.detail(id) });
                qc.invalidateQueries({ queryKey: statusPageKeys.all });
            },
        },
    );
}
