...
export function VoximplantManagement() {
  ...
  const {
    ...
    error: historyError,
  } = useVoxCallHistory(providerId, !!providerId);
  ...
  const openSessionLog = async (sessionId: number) => {
    ...
    try {
      const res = await voximplantApi.getSessionLog(providerId, sessionId);
      if (!res.success || !res.data) {
        throw new Error(res.message || t("voximplant.sessionLogFailed"));
      }
      setLogText(res.data.content);
      setLogTruncated(res.data.truncated);
    } catch (err) {
      setLogError(err instanceof Error ? err.message : t("voximplant.sessionLogFailed"));
    } finally {
      setLogLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Card className="p-6 bg-card/80 backdrop-blur-sm border-border">
        <div className="flex items-start gap-4 mb-6">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              <h3 style={{ fontSize: "1.125rem", fontWeight: 600 }}>
                {t("voximplant.callSessions")}
              </h3>
              <Badge className="bg-muted/10 text-muted-foreground border-muted/20 border">
                {t("voximplant.last24Hours")}
              </Badge>
            </div>
            <p style={{ fontSize: "0.875rem", color: "#94A3B8" }}>
              {t("voximplant.callSessionsDesc")}
            </p>
          </div>
          <Button
            ...
            className="bg-input-background"
          >
            <RefreshCw className={`w-4 h-4 mr-2 ${historyFetching ? "animate-spin" : ""}`} />
            {t("common.refresh")}
          </Button>
        </div>
        ...
        {historyLoading ? (
          ...
          <div className="p-4 rounded-lg bg-error-500/10 border border-error-500/20 text-sm text-error-500">
            {historyError instanceof Error ? historyError.message : t("voximplant.callHistoryFailed")}
          </div>
        ) : (callHistory?.sessions?.length ?? 0) === 0 ? (
          <p className="text-center text-sm text-muted-foreground py-8">
            {t("voximplant.noCallSessions")}
          </p>
        ) : (
          <div className="space-y-3">
            {callHistory!.sessions.map((session) => {
              const remote = session.calls.map((c) => c.remoteNumber).filter(Boolean).join(", ");
              return (
                <div
                  key={session.callSessionHistoryId}
                  className="flex items-start gap-4 p-4 rounded-lg border border-border bg-surface-light/10"
                >
                  <div className="flex-1 min-w-0 space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="text-sm font-semibold">
                        #{session.callSessionHistoryId}
                      </p>
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    disabled={!session.hasLog}
                    onClick={() => openSessionLog(session.callSessionHistoryId)}
                    className="bg-input-background shrink-0"
                  >
                    <FileText className="w-4 h-4 mr-2" />
                    {t("voximplant.viewLog")}
                  </Button>
                </div>
              );
            })}
            <p className="text-xs text-muted-foreground pt-1">
              {t("voximplant.showingSessions", {
                shown: String(callHistory!.sessions.length),
                total: String(callHistory!.totalCount),
              })}
            </p>
          </div>
        )}
      ...
      <Dialog open={logOpen} onOpenChange={setLogOpen}>
        <DialogContent className="bg-card border-border sm:max-w-3xl max-h-[85vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>
              {t("voximplant.sessionLogTitle")}
              {logSessionId != null ? ` #${logSessionId}` : ""}
            </DialogTitle>
            <DialogDescription>{t("voximplant.sessionLogDesc")}</DialogDescription>
          </DialogHeader>
          {logLoading ? (
            ...
            <p className="text-sm text-error-500 py-4">{logError}</p>
          ) : (
            <>
              {logTruncated && (
                <p className="text-xs text-warning-500 mb-2">{t("voximplant.sessionLogTruncated")}</p>
              )}
              <pre className="text-xs whitespace-pre-wrap break-words overflow-auto max-h-[60vh] rounded-md bg-surface-light/20 p-4 border border-border">
                {logText || "—"}
              </pre>
            </>
          )}
