...
import { useProfile, useAdoptAccountLanguage } from "@/features/profile/hooks/use-profile";
...
const NAV_DEFS: NavDef[] = [
  ...
  { labelKey: "nav.diagnostics", href: "/diagnostics", icon: Activity },
...
type VisibleNav = NavDef & { selfVisible: boolean };

// Drops the entries the signed-in role cannot open — UX, not access control. A group heading travels
// to the first surviving entry of its group, so filtering the leader does not orphan the rest.
function visibleNavItems(role: string | null | undefined): VisibleNav[] {
  ...
  for (const item of NAV_DEFS) {
    if (item.groupKey) {
      ...
      groupHeadingUsed = false;
    }

    const subItems = item.subItems?.filter((sub) => canAccessRoute(role, sub.href));
    const selfVisible = canAccessRoute(role, item.href);

    if (!selfVisible && !subItems?.length) continue;

    items.push({
      ...item,
      subItems,
      selfVisible,
      groupKey: groupHeadingUsed ? undefined : groupKey,
    });
...
function NavItems({ showLabels, onNavigate }: NavItemsProps) {
  ...
  useAdoptAccountLanguage(profile?.culture);
...
export function AppShell({ children }: AppShellProps) {
  ...
  // `children` is the same element reference across shell re-renders, so React skips that
  // subtree — nav labels would switch language while the page content stayed stale. Keying
  // <main> by locale remounts the page on a language change (same effect as a navigation).
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className={`transition-all duration-300 ${isSidebarOpen ? "lg:ml-64" : "lg:ml-20"}`}>
        ...
        <main id="main-content" key={locale} className="min-h-[calc(100vh-4rem)]" tabIndex={-1}>
          {children}
        </main>
      </div>
    </div>
